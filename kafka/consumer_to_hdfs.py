...
def save_to_hdfs(data: list, hdfs_path: str, topic: str):
    ...
    # Upload ke HDFS
    try:
        # 1. Copy file dari lokal/WSL ke dalam container namenode
        container_tmp_file = f'/tmp/{topic}_{timestamp}.json'
        subprocess.run(
            ['docker', 'cp', local_file, f'namenode:{container_tmp_file}'],
            check=True, capture_output=True
        )
        
        # 2. Pindahkan file yang sudah ada di dalam container ke HDFS
        result = subprocess.run(
            ['docker', 'exec', 'namenode', 'hdfs', 'dfs', '-put', '-f',
             container_tmp_file, hdfs_file],
            capture_output=True, text=True, timeout=30
        )
        
        # 3. Bersihkan file sampah di dalam container namenode
